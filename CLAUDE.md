...
| `backend/app/agent.py` | Agent spawning, stdout/stderr capture, status tracking via `parse_status()` (checks structured `cronos_status` block first, falls back to deprecated free-text `STATUS:` line with warning log) |
