...
| `packages/delivery-workflow/lib/contract.py` | **Canonical contract module** — pure data: Final constants (CC_VERSION, HEADER_FIELDS, STATUS_VALUES, R_RULES, etc.). Zero app imports. Single source of truth for the delivery gate contract |
| `packages/delivery-workflow/lib/verify.py` | **Canonical artifact verifier** — core validation engine for schema-class artifacts; SCHEMAS_DIR anchored to `Path(__file__).resolve().parent / "schemas"` (lib/schemas/). Imports use `lib.contract`. The `--normalize` CLI flag lazily imports a normalizer and degrades gracefully (EXIT_RETRY) when one is not installed |
| `packages/delivery-workflow/lib/gate.py` | **Gate engine** — `runGate(gate, artifact_paths, ...) → GateResult`; contract checks + outcome re-execution for delivery-workflow gate nodes. Imports only `lib.verify` / `lib.security` (no `app.*`). Consumed by the Cronos adapter (`adapters/cronos/adapter.py`) |
| `packages/delivery-workflow/lib/schemas/` | **Canonical schemas** — single source of truth for the 8 schema YAML files (research, analysis, design, implementation, test, review, doc, retro). Resolves via `verify.SCHEMAS_DIR` |
