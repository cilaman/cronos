...
| `backend/app/harnesses/state_mapping.py` | Bidirectional mapping between RunState (Cronos) and WorkflowState (delivery-workflow); `runstate_to_workflowstate()` and `workflowstate_to_runstate()` pure functions; preserves loop bookkeeping and round-trip fidelity via fields dict sentinel |
| `backend/app/harnesses/executor_adapter.py` | HarnessExecutorAdapter implementing ExecutorInterface protocol for delivery-workflow runner; wraps agent dispatch (WorkerAdapter stub), condition evaluation (via harnesses.decision), and escalation discrimination (human-wait vs loop-exhaust); telemetry event synthesis for SSE; entry point for runner-path harness execution |
...
| `backend/app/run_executor.py` | RunExecutor driving harness execution via worker; `CRONOS_HARNESS_RUNNER` env flag selects between BFS (default) and runner (via compiler/adapter) paths; helper functions for executor-variant persistence (backward-compatible JSON serialization); handles initial-run vs resume lifecycle |
...
| `backend/app/pipeline/contract.py` | **Re-export stub** (SG7) — thin wrapper doing `from lib.contract import *` to maintain backward compatibility for 5 existing backend callers; canonical source is `packages/delivery-workflow/lib/contract.py`. Exports: CC_VERSION, HEADER_FIELDS, HEADER_REQUIRED_FIELDS, STATUS_VALUES, NEXT_CONSUMER_USER_SENTINEL, AGENT_REPORTED_METRICS, TRACE_OWNED_METRICS, REQUIRED_SECTIONS, FINDINGS_SECTION_ALIASES, OPEN_QUESTIONS_SECTION_ALIASES, R_RULES, ARTIFACT_PATH_TEMPLATE, NO_PROSE_PARSING_RULE |
| `backend/app/pipeline/verify.py` | **Re-export stub** (SG7) — thin wrapper importing 12 public symbols from `lib.verify`; preserves `if __name__ == "__main__"` guard for `python -m app.pipeline.verify` CLI tests; canonical source is `packages/delivery-workflow/lib/verify.py`. The deferred `from app.pipeline.normalize import normalize` at line 1350 is an accepted residual (CLI-only, deferred to StandaloneAdapter SG) |
| `packages/delivery-workflow/lib/contract.py` | **Canonical CC-v1 contract module** (SG7) — pure data: Final constants (CC_VERSION, HEADER_FIELDS, STATUS_VALUES, R_RULES, etc.). Zero app imports. Single source of truth for the contract that was originally at `backend/app/pipeline/contract.py` |
| `packages/delivery-workflow/lib/verify.py` | **Canonical CC-v1 verifier** (SG7) — core validation engine lifted from `backend/app/pipeline/verify.py`; SCHEMAS_DIR re-anchored to `Path(__file__).resolve().parent / "schemas"` resolving to lib/schemas/. Imports rewritten to use `lib.contract`; retained deferred CLI-only `from app.pipeline.normalize import normalize` at line 1350 (documented as follow-up) |
| `packages/delivery-workflow/lib/schemas/` | **Canonical CC-v1 schemas** (SG7) — single source of truth for 8 schema YAML files (research, analysis, design, implementation, test, review, doc, retro). Lifted verbatim from `backend/app/pipeline/schemas/` in I2; backend copies deleted. Resolves via `verify.SCHEMAS_DIR` |
