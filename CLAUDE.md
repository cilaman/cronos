...
| `backend/app/harnesses/compiler.py` | Pure `compile(harness: Harness) → IRGraph` translating Cronos harness model to portable IR consumed by delivery-workflow runner; wait-node disambiguation via mode→kind table, LoopPolicy construction, run-state bootstrap |
| `backend/app/harnesses/state_mapping.py` | **Reduced role since R10d** — node/run-level vocabulary translation between RunState (Cronos REST/SSE persistence, frontend overlay) and WorkflowState (delivery-workflow), used for UI state + runner-path resume rebuild ONLY; terminal interpretation flows through the shared Outcome table in `app.delivery_outcomes`, never these tables |
| `backend/app/harnesses/executor_adapter.py` | HarnessExecutorAdapter implementing the package `NodeExecutor` port (agent dispatch via WorkerAdapter stub) + `HostPort` (`on_event` writes run-level blocked/escalated); telemetry event synthesis for SSE; entry point for runner-path harness execution. Condition evaluation is runner-internal since R10b (no evalCondition) |
| `backend/app/harnesses/executor.py` | **Harness executor (dual-path)** — runtime-gated BFS (breadth-first search) DAG interpreter **OR** delivery-workflow runner (via compiler+adapter) selected by `CRONOS_HARNESS_RUNNER` env flag; control-flow dispatch (decision/wait/aggregator nodes), agent invocation, fail-fast on node failure, variable scope propagation, run-state persistence, SSE event publishing (`node_transition`, `edge_chosen`, `run_status`), cancel-race guard |
| `backend/app/run_executor.py` | RunExecutor driving harness execution via worker; `CRONOS_HARNESS_RUNNER` env flag selects between BFS (default) and runner paths — the runner path drives the package `DeliveryRun` facade and finalizes the tracking task via the shared Outcome table (`app.delivery_outcomes`, R10d); helper functions for executor-variant persistence (backward-compatible JSON serialization); handles initial-run vs resume lifecycle |
| `backend/app/harnesses/decision.py` | Decision node evaluator — four-layer signal precedence (STATUS marker > exit_reason > regex > variable condition) with whitelisted variable grammar (==, !=, in); supports dotted-path scope routing (e.g., `review.fields.verdict`) |
...
| `backend/app/delivery_driver.py` | **Delivery-workflow host driver** — detects the `<!-- delivery-workflow: spec -->` sentinel, builds the CronosAdapter, drives the package `DeliveryRun` facade (start/resume), translates the persisted status + user action into one typed resume event (R7 grammar), and finalizes the goal via the shared Outcome table. Zero heuristics, zero sidecar files, no WorkflowState internals |
| `backend/app/delivery_adapter.py` | **Cronos adapter (host-owned since R10c)** — `CronosAdapter` implements the package `NodeExecutor` (dispatchAgent/runGate/runExec) + `HostPort` (on_event parks the tracking task) ports; `CronosStateOps` re-exports the package `lib.state.ops.StateStoreOps` verbatim. dispatchAgent maps `trace.node_status` through the package closed-vocabulary boundary (`results.agent_result_from_envelope`); runExec delegates to `lib.exec_node.run_exec_command` |
| `backend/app/delivery_outcomes.py` | **The shared Outcome → TaskState table (R10d, kills D16)** — `action_for_outcome()` implements the §5.6 five-row mapping (+ cancelled/running), `apply_outcome_to_task()` is the single applier BOTH hosts (delivery driver and harness runner path) call, `render_stall_message()` renders the run-level stall record |
| `packages/delivery-workflow/` | **Portable delivery-workflow package** (`pip install -e packages/delivery-workflow`, importable as `delivery_workflow`, src layout). Hosts consume ONLY the `DeliveryRun` facade + `Outcome`/`RunEvent`; dependencies point app → package, never back (import-linter enforced). Standalone CLI: `python -m delivery_workflow run spec.yaml --workdir DIR` |
| `packages/delivery-workflow/src/delivery_workflow/delivery_run.py` | **DeliveryRun facade** — the ONLY host surface: `start()` / `resume(event)` / `outcome()` (pure read) / `cancel()`, all returning the closed `Outcome` taxonomy |
| `packages/delivery-workflow/src/delivery_workflow/outcome.py` | Closed run-outcome taxonomy (01 §5.6: done/stalled/failed/blocked/escalated/cancelled + pure-read running); `outcome_from_state()` derives it purely from WorkflowState so hosts never read node internals |
| `packages/delivery-workflow/src/delivery_workflow/events.py` | Typed `RunEvent` grammar (NodeStarted/NodeFinished/RunBlocked/RunStalled/RunFailed/RunEscalated) delivered to `HostPort.on_event`; `NullHost`, `safe_emit` |
| `packages/delivery-workflow/src/delivery_workflow/local_executor.py` | **Reference runtime (R10e)** — `LocalProcessExecutor` spawns `claude -p <brief>` per agent node, parses the node_status fence with `lib.node_status`, closes the vocabulary via the same `agent_result_from_envelope` mapping the Cronos adapter uses; `LocalHostPort` prints JSON progress lines. Behind the standalone CLI (`__main__.py`) with honest per-Outcome exit codes |
| `packages/delivery-workflow/src/delivery_workflow/runner/` | Invariant-checked work-list walker (core/dispatch/loop/scope) + `resume.py` (typed re-entry grammar: HumanAnswer/RetryFailed/RaiseBudget/Nothing) |
| `packages/delivery-workflow/src/delivery_workflow/lib/state/` | StateStore/EventLog primitives, `ops.StateStoreOps` (the one StateOps implementation), `conformance.py` round-trip laws every StateOps must satisfy |
| `packages/delivery-workflow/src/delivery_workflow/lib/contract.py` | **Canonical contract module** — pure data: Final constants (CC_VERSION, HEADER_FIELDS, STATUS_VALUES, R_RULES, etc.). Zero app imports. Single source of truth for the delivery gate contract |
| `packages/delivery-workflow/src/delivery_workflow/lib/verify.py` | **Canonical artifact verifier** — core validation engine for schema-class artifacts; SCHEMAS_DIR anchored to `Path(__file__).resolve().parent / "schemas"` (lib/schemas/). Imports use `lib.contract` |
| `packages/delivery-workflow/src/delivery_workflow/lib/gate.py` | **Gate engine** — `runGate(gate, artifact_paths, ...) → GateResult`; contract checks + outcome re-execution for delivery-workflow gate nodes. Imports only `lib.verify` / `lib.security` (no `app.*`). Consumed by the host adapter (`backend/app/delivery_adapter.py`) and `LocalProcessExecutor` |
| `packages/delivery-workflow/src/delivery_workflow/lib/schemas/` | **Canonical schemas** — single source of truth for the 8 schema YAML files (research, analysis, design, implementation, test, review, doc, retro). Resolves via `verify.SCHEMAS_DIR` |
