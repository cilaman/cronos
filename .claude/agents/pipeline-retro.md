...
- **Retro -> memory write-back (task 4.3)**: the `pipeline-gate` skill's Step 3b calls `python -m app.pipeline.retro_memory_writer --space ${SPACE_DIR} --slug ${GOAL_SLUG}` automatically when the retro gate passes (VERIFY_EXIT=0). This writes ALL findings (any fix_type) as global Cronos memory items (kind=procedure for agent_prompt_refinement, kind=observation for all others) so they surface via `app.memory_retrieval.retrieve` in future pipeline runs in ANY space.
