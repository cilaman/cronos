...
describe("SpaceToolsPage — PageHeader text-title migration", () => {
  beforeEach(() => {
    mockUnadoptMutate.mockReset();
    mockUnadoptIsPending = false;
    mockSpaceTools = makeTools([]);
  ...
  it("renders the space name as the h1 title when a space is active", () => {
    renderPage("space-1");
    const h1 = document.querySelector("h1");
    expect(h1?.textContent).toBe("Test Space");
  });

  it("renders 'Inventory' as the h1 title when no space is selected", () => {
    render(
    ...
    const h1 = document.querySelector("h1");
    expect(h1?.textContent).toBe("Inventory");
  });
});
