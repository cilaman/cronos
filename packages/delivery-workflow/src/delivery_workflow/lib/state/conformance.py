...
Every ``StateOps`` implementation — the package's ``StateStoreOps``, the
...
``STATEOPS_CONFORMANCE_CHECKS``. Host test suites parametrize over the list:
    ...
    from delivery_workflow.lib.state.conformance import STATEOPS_CONFORMANCE_CHECKS
...
def check_top_level_status_roundtrip(make_ops: MakeOps) -> None:
    ...
    ``cancelled`` (R10b: written by ``DeliveryRun.cancel``) is part of the
    law — a StateOps that drops it would let a cancelled run resume.
    """
    ops = make_ops(_fresh_state())
    for status in (
        "blocked", "failed", "escalated", "stalled", "cancelled", "running", "done"
    ):
