...
from delivery_workflow.events import RunEscalated, safe_emit
...
if TYPE_CHECKING:
    ...
    from delivery_workflow.interface import HostPort, StateOps
...
def should_loop_back(
    ...
    *,
    ...
    Evaluates ``node.loop.until`` against *scope* — runner-internally via
    ``lib.conditions.eval_condition`` (R10b: condition evaluation left the
    executor surface; the ``eval_condition`` keyword exists for scripted
    simulations only).  If the condition is True, the loop exits (no
    loop-back).  If False and the attempt count has not reached the max, the
    node is re-enqueued.
    ...
    - Emits ``RunEscalated(kind='loop')`` to *host* or stops (per on_exhaust).
    ...
    if loop.until:
        try:
            until_met = _eval(loop.until, scope)
        except Exception as exc:
            log.warning("should_loop_back: eval_condition raised %s — defaulting to False", exc)
