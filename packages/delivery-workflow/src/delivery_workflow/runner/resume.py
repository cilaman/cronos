...
Since R10b this entry point is wrapped by the ``DeliveryRun`` facade
(``delivery_workflow.delivery_run``) — hosts call ``DeliveryRun.resume(event)``
and receive an ``Outcome``; this module stays importable as the underlying
event grammar + state-transition engine.  A ``cancelled`` run (written only by
``DeliveryRun.cancel``) is terminal: every event raises ``ResumeError``.
...
from delivery_workflow.events import RunStalled, safe_emit
from delivery_workflow.ir import IRGraph, IRNode
...
def resume(
    ...
    *host* (R10b) receives typed RunEvents from the resumed run — including a
    ``RunStalled`` when the event itself stalls the run (reject without route,
    retry ceiling).  *eval_condition* is the scripted-simulation override
    passed through to ``run()`` (never a host port).
    ...
    if state.status == "cancelled":
        raise ResumeError(
            "the run was cancelled (DeliveryRun.cancel) — cancelled runs are "
            "terminal and cannot be resumed; start a fresh run"
        )

    if isinstance(event, HumanAnswer):
        proceed = _apply_human_answer(graph, state_ops, state, event, host=host)
    elif isinstance(event, RetryFailed):
        proceed = _apply_retry_failed(
            graph, state_ops, state, event, max_retries, host=host
        )
    ...
    from delivery_workflow.runner.core import run
...
def _rearm_nodes(
    ...
    *,
    ...
    """
    from delivery_workflow.runner.core import (
        _graph_fingerprint,
        _load_edge_record,
