...
Harvests all 8 node-kind dispatch patterns from HarnessExecutor:
  ...
  human      → emit RunBlocked(node, question) + park (blocked)
  decision   → edge-routing only; no external dispatch; always "done"
  wait(human)→ emit RunBlocked(node, question) + park (blocked)
  wait(timed)→ emit RunEscalated(kind='timed_wait') (sleep deferred to host)
  aggregator → inspect predecessor NodeState; all/any verdict
  trigger    → immediate done (fires once)

R10b port split: the executor is a NodeExecutor (dispatchAgent/runGate/
runExec ONLY); host notification flows through the optional HostPort as
typed RunEvents (delivery_workflow.events) — the executor-level escalate
reason-string channel is gone.
...
from delivery_workflow.events import RunBlocked, RunEscalated, safe_emit
from delivery_workflow.ir import IRNode
from delivery_workflow.state_types import WorkflowState

if TYPE_CHECKING:
    from delivery_workflow.interface import HostPort, NodeExecutor
...
class NodeOutcome:
    ...
    (host adapters, test doubles) return result objects and never touch
...
def dispatch_node(
    ...
    *incoming* maps node_id → predecessor node ids (from the graph's edges).
    ...
    return handler(
        node=node, scope=scope, executor=executor, state=state, attempt=attempt,
        host=host,
    )
...
def _dispatch_agent(
    ...
    The executor.dispatchAgent may be async (a host adapter) or sync (NullRuntime
...
def _dispatch_human(
    ...
    """Dispatch a human node — emit RunBlocked (park the run for a HumanAnswer)."""
    prompt: str = node.data.get("prompt", "Human input required.")
    safe_emit(host, RunBlocked(node_id=node.id, question=prompt))
...
def _dispatch_wait(
    ...
    human mode  → emit RunBlocked + block (mirrors HarnessExecutor enter_wait).
    timed mode  → emit RunEscalated(kind='timed_wait') (sleep deferred to host).
    """
    ...
    safe_emit(host, RunEscalated(
        kind="timed_wait",
        node_id=node.id,
        detail=f"max_wait_seconds={max_wait}",
    ))
