...
condition-aware seeding routes the re-armed frontier.  ``cancelled`` is
written by exactly one caller — ``DeliveryRun.cancel()`` (R10b) — and is
terminal: ``resume()`` refuses it.

Port split (R10b, 02-package-boundary.md §2.2): the runner consumes a
``NodeExecutor`` (dispatchAgent/runGate/runExec ONLY) and notifies an
optional ``HostPort`` with typed ``RunEvent``s (``delivery_workflow.events``)
— replacing the executor-level ``escalate(node_id, reason)`` hook and its
host-side reason-prefix sniffing.  Edge/loop condition evaluation is
RUNNER-INTERNAL: ``when``/``until`` expressions are evaluated by
``delivery_workflow.lib.conditions.eval_condition`` directly (the harness
grammar in ``app.harnesses.decision`` delegates to that same module, so
harness edge semantics are identical by construction).  The ``eval_condition``
keyword exists for scripted simulations (conformance harness, repro script) —
it is NOT a host port; hosts never override it.
...
from delivery_workflow.lib.conditions import eval_condition as _lib_eval_condition
...
def run(
    ...
    *,
    ...
    executor:
        A concrete NodeExecutor (dispatchAgent/runGate/runExec — R10b port
        split; a host adapter or a NullRuntime subclass).
    state_ops:
        Optional StateOps for persistence and cancel-race detection.  When
        None a simple in-memory state is used (unit-test mode).
    host:
        Optional HostPort; receives typed RunEvents (node_started/
        node_finished/run_blocked/run_stalled/run_failed/run_escalated).
        Delivery is fire-and-forget (``events.safe_emit``).
    eval_condition:
        Condition evaluator override for scripted simulations ONLY (defaults
        to the runner-internal ``lib.conditions.eval_condition``).  Not a
        host port — see the module docstring.
    ...
    """
    from delivery_workflow.runner.dispatch import NodeOutcome, dispatch_node
    ...
    _eval = eval_condition if eval_condition is not None else _lib_eval_condition
        ...
        for node_id in settled_node_ids:
            ...
            for edge_idx, edge in outgoing.get(node_id, []):
                ...
                if edge.when == "" or edge.when is None:
                    ...
                    condition_met = _eval(edge.when, resume_scope)
                if condition_met:
    ...
    while work_list:
        ...
        if global_iterations >= global_cap:
            ...
            safe_emit(host, RunEscalated(
                kind="iteration_cap",
                node_id="__runner__",
                detail="global_iteration_cap_exceeded",
            ))
        ...
        _pre_ns = state.nodes.get(node_id)
        safe_emit(host, NodeStarted(
            node_id=node_id,
            attempt=(_pre_ns.attempt if _pre_ns is not None else 0) + 1,
        ))
        ...
        safe_emit(host, NodeFinished(node_id=node_id, status=ns.status))
        ...
        if outcome.status == "failed":
            ...
            _err = (outcome.fields or {}).get("error")
            safe_emit(host, RunFailed(
                node_id=node_id,
                reason=str(_err) if _err else f"node '{node_id}' failed",
            ))
            state.status = "failed"
            if state_ops is not None:
        ...
        if node.loop is not None:
            if node.kind == "gate":
                ...
                if decision != "proceed" and ns.attempt >= node.loop.max:
                    ...
                    # their park message from state.stall alone.  The host is
                    # notified via RunStalled (typed detail) — deliberately NOT
                    # RunEscalated, which would park a generic message over
                    # the actionable one.
                    #
                    ...
                    safe_emit(host, RunStalled(detail=stall))
                ...
                loop_back = should_loop_back(
                    node, state, updated_scope, host=host, eval_condition=_eval
                )
                if not loop_back and ns.status == "escalated":
                    # Loop exhausted with on_exhaust='escalate' (R7):
                    # should_loop_back emitted RunEscalated(kind='loop') and
                    # mutated the node's IN-MEMORY status to 'escalated' —
                    # persist that status AND terminate the run 'escalated',
                    # overriding any 'blocked' the host's on_event hook may
                    # have written (the Cronos adapter parks 'blocked' for
                    # every escalation cause).  Without this write the run persisted 'blocked'
    ...
    if stall_record is not None:
        ...
        safe_emit(host, RunStalled(detail=stall_record))
        state.status = "stalled"
...
def _enqueue_successors(
    ...
    work_list: list[str],
    eval_condition: "Callable[[str, dict[str, Any]], bool]",
    scope: dict[str, Any],
    state_ops: "StateOps | None" = None,
) -> None:
    ...
    """
    from delivery_workflow.runner.loop import reset_downstream_nodes

    for edge_idx, edge in outgoing.get(node_id, []):
        target = edge.target

        # Evaluate edge condition (runner-internal, R10b — see module docstring).
        if edge.when == "" or edge.when is None:
            condition_met = True
        else:
            condition_met = eval_condition(edge.when, scope)
