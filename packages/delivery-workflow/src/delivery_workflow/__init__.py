...
- ``load_agent_definition`` / ``load_skill_definition`` /
  ``paired_skill_section`` / ``return_contract`` / ``upstream_scope_section``
  (+ ``AGENTS_DIR`` / ``SKILLS_DIR`` / ``PAIRED_SKILLS``) — the shared
  brief-composition helpers every brief composer builds from.
