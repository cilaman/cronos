...
Run `classify_findings()` from `delivery_workflow.lib.improve` on `fields.findings[]` to partition
every finding into `tier0 / tier1 / tier2` (DD-003, fix_type-authoritative):

```python
from delivery_workflow.lib.improve import classify_findings
...
```sh
python -m delivery_workflow.lib.evals --repo-root <repo_root>
```

Or from Python:

```python
from delivery_workflow.lib.evals import run_eval_corpus
...
```python
from delivery_workflow.lib.improve import run_back_half
back = run_back_half(
    routed.tier1,
    routed.tier2,
