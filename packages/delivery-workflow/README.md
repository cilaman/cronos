...
├── adapters/
│   └── cronos/              # Cronos-specific wiring (Phase 6+)
│
...
### Cronos (Phase 6.1 — Current)

The Cronos backend (`backend/app/`) adopts delivery/v1:

1. **Agent dispatch** → spawns `claude code -s <space>` subprocess; captures stdout/stderr
2. **Gate verification** → invokes `backend/app/pipeline/verify.py`
3. **Condition evaluation** → harness decision logic (Phase 3)
4. **Telemetry wiring** → `backend/app/run_side_effects.py` calls `lib.telemetry.TelemetrySink.emit()` after each agent/gate; transient sink (Phase 6 adds `StateStore` persistence to `state.json`)
5. **State management** → `backend/app/pipeline/state_writer.py` bridges `TelemetrySink` output to `PhaseMetrics` for pipeline-state.json

Key adapter in `packages/delivery-workflow/adapters/cronos/__init__.py` wires the interfaces.
