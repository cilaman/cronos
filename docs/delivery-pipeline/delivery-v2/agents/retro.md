...
tools: Read, Grep, Glob, Bash, Write   # NO Edit by design — read-only. Write is for its own retro artifact only. Bash is for read-only inspection.
---
...
**Load the `retro` skill before retrospecting.** It carries the method: how to read delivery/v1
run state, the five-dimension scoring rubric, the tier/fix_type decision tree, the severity
ladder, the finding format, and the artifact structure. This definition holds only your role,
inputs, and the hard rules.
