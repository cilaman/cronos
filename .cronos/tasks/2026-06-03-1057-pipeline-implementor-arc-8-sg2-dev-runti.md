...
depends_on:
- 2026-06-03-1057-pipeline-architect-arc-8-sg2-dev-runtime
id: 2026-06-03-1057-pipeline-implementor-arc-8-sg2-dev-runti
manual_order: 0
...
state: backlog
title: 'pipeline-implementor: Arc 8/SG2 — dev runtime process manager + health poller'
type: task
...
# Phase 4 — implementation: Arc 8/SG2 — dev runtime process manager + health poller

Goal slug: `arc8-dev-process` · Pipeline dir: `.cronos/pipeline/arc8-dev-process/` · Sub-agent: `pipeline-implementor`.
...
slugs are `arc8-dev-process--<iter_id_lower>` (e.g. `arc8-dev-process--i1`).
...
Read `.cronos/pipeline/arc8-dev-process/design-report-arc8-dev-process.md`'s YAML header. Extract
...
1. **Spawn the implementor** via the `Agent` tool, `subagent_type="pipeline-implementor"`:
   ...
   ```text
   slug                 = arc8-dev-process--<iter.id.lower()>
   space                = $SPACE_DIR
   design_report_path   = .cronos/pipeline/arc8-dev-process/design-report-arc8-dev-process.md
   iteration_id         = <iter.id>
   prior_iteration_results = [<paths to impl-report-arc8-dev-process--*.md for satisfied deps>]
   ```
