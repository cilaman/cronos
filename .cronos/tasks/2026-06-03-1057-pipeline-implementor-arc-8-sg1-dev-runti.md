...
depends_on:
- 2026-06-03-1057-pipeline-architect-arc-8-sg1-dev-runtime
id: 2026-06-03-1057-pipeline-implementor-arc-8-sg1-dev-runti
manual_order: 0
...
state: backlog
title: 'pipeline-implementor: Arc 8/SG1 — dev_runtime space schema + port allocator'
type: task
...
# Phase 4 — implementation: Arc 8/SG1 — dev_runtime space schema + port allocator

Goal slug: `arc8-dev-schema` · Pipeline dir: `.cronos/pipeline/arc8-dev-schema/` · Sub-agent: `pipeline-implementor`.
...
slugs are `arc8-dev-schema--<iter_id_lower>` (e.g. `arc8-dev-schema--i1`).
...
Read `.cronos/pipeline/arc8-dev-schema/design-report-arc8-dev-schema.md`'s YAML header. Extract
...
1. **Spawn the implementor** via the `Agent` tool, `subagent_type="pipeline-implementor"`:
   ...
   ```text
   slug                 = arc8-dev-schema--<iter.id.lower()>
   space                = $SPACE_DIR
   design_report_path   = .cronos/pipeline/arc8-dev-schema/design-report-arc8-dev-schema.md
   iteration_id         = <iter.id>
   prior_iteration_results = [<paths to impl-report-arc8-dev-schema--*.md for satisfied deps>]
   ```
