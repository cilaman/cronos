...
depends_on:
- 2026-06-03-1057-pipeline-architect-arc-8-sg3-dev-runtime
id: 2026-06-03-1057-pipeline-implementor-arc-8-sg3-dev-runti
manual_order: 0
...
state: backlog
title: 'pipeline-implementor: Arc 8/SG3 — dev runtime API + SSE log stream'
type: task
...
# Phase 4 — implementation: Arc 8/SG3 — dev runtime API + SSE log stream

Goal slug: `arc8-dev-api` · Pipeline dir: `.cronos/pipeline/arc8-dev-api/` · Sub-agent: `pipeline-implementor`.
...
slugs are `arc8-dev-api--<iter_id_lower>` (e.g. `arc8-dev-api--i1`).
...
Read `.cronos/pipeline/arc8-dev-api/design-report-arc8-dev-api.md`'s YAML header. Extract
...
1. **Spawn the implementor** via the `Agent` tool, `subagent_type="pipeline-implementor"`:
   ...
   ```text
   slug                 = arc8-dev-api--<iter.id.lower()>
   space                = $SPACE_DIR
   design_report_path   = .cronos/pipeline/arc8-dev-api/design-report-arc8-dev-api.md
   iteration_id         = <iter.id>
   prior_iteration_results = [<paths to impl-report-arc8-dev-api--*.md for satisfied deps>]
   ```
