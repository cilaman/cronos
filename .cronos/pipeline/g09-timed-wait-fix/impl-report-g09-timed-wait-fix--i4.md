...
Validation: `cd backend && pytest tests/test_harness_wait.py tests/test_harness_executor.py -k "timed_wait or TimedWait" --no-cov -q` — 8 tests passed. Full suite: 2799 passed (126s).
...
- `test_harness_run_state.py` required no new tests: `wake_at` round-trips automatically
  via existing `asdict`/`from_dict` machinery; backward-compat covered by existing
  `test_node_state_timing_backwards_compat`.
...
```
All 8 G09-specific tests should pass. Also validate the full suite:
```
