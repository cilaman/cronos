...
Gate run for goal `gui-polish` in space `cronos-development`. 2943 tests passed, 663 failed, 836 errored, 0 skipped. Coverage: 50.5%. Gate decision: **FAIL**.
...
- Backend API POST returned 401 (auth required); report was persisted directly via TestReportStore on disk.
- Frontend vitest: 1464 tests, 0 failures (fresh run completed Jun 23 20:24).
- Backend pytest: 2978 tests collected; coverage=50.48% (below 80% floor — pre-existing deficit, not introduced by gui-polish).
...
Gate result: **FAIL** — 2943p / 663f / 836e, coverage 50.5%.
