...
def test_cc_delivery_from_report_maps_frontmatter():
    """Regression for 'scout DONE but node failed': a CC-v1 report (YAML
    frontmatter, no delivery_status fence) maps to a done delivery dict with
    routing fields (has_ui) exposed under fields."""
    from datetime import UTC, datetime, timedelta
    from app.run_executor import _cc_delivery_from_report

    def _pipeline(tmp):
        d = tmp / ".cronos" / "pipeline" / "sg1-slug"
        d.mkdir(parents=True)
        (d / "analysis-report-sg1-slug.md").write_text(
            "---\n"
            "cc_version: '1.0'\n"
            ...
            "status: done\n"
            "outputs_produced:\n"
            "  - .cronos/pipeline/sg1-slug/analysis-report-sg1-slug.md\n"
            "has_ui: false\n"
            "blockers: []\n"
            "---\n\n## Summary\n\nx\n"
        )
        return tmp / ".cronos" / "pipeline"

    import tempfile
    with tempfile.TemporaryDirectory() as td:
        root = _pipeline(Path(td))
        since = datetime.now(tz=UTC) - timedelta(seconds=1)
        d = _cc_delivery_from_report(root, since, {"class": "analysis"})

    assert d is not None
    assert d["status"] == "done"
    assert d["produces"] == "analysis"
    assert d["artifact_paths"] == [".cronos/pipeline/sg1-slug/analysis-report-sg1-slug.md"]
    # Routing field exposed so `analyze.fields.has_ui == false` resolves.
    assert d["fields"]["has_ui"] is False


def test_cc_delivery_from_report_ignores_stale_report():
    """A report older than the child start (previous phase/run) is not picked up."""
    from datetime import UTC, datetime, timedelta
    from app.run_executor import _cc_delivery_from_report
    import tempfile, os

    with tempfile.TemporaryDirectory() as td:
        d = Path(td) / ".cronos" / "pipeline" / "s"
        d.mkdir(parents=True)
        f = d / "scout-report-s.md"
        f.write_text("---\nstatus: done\n---\n")
        old = 1_000_000.0  # ancient mtime
        os.utime(f, (old, old))
        since = datetime.now(tz=UTC)
        assert _cc_delivery_from_report(Path(td) / ".cronos" / "pipeline", since, None) is None


@pytest.mark.asyncio
async def test_run_delivery_child_bridges_cc_report(tmp_path):
    """End-to-end: an agent that writes a CC-v1 report (no fence) yields a done
    delivery dict — the exact scout case that used to fail the node."""
    goal = SimpleNamespace(id="goal-1", space_id="sp1", title="G", brief="")
    child = SimpleNamespace(
        id="child-1", space_id="sp1", title="[delivery] scout",
    ...
    ex, bus, worker = _make_executor(store, worker=worker)
    ex.space_store = SimpleNamespace(spaces_dir=tmp_path, get=lambda sid: None)

    async def _fake_run_agent(child_task, **kwargs):
        # The agent writes its CC-v1 report to the pipeline dir.
        d = tmp_path / "sp1" / ".cronos" / "pipeline" / "g-slug"
        d.mkdir(parents=True, exist_ok=True)
        (d / "scout-report-g-slug.md").write_text(
            "---\nstatus: done\nphase: scout\n"
            "outputs_produced:\n  - .cronos/pipeline/g-slug/scout-report-g-slug.md\n"
            "blockers: []\n---\n\n## Summary\n\nok\n"
        )
        return SimpleNamespace(status=SimpleNamespace(value="done"))

    with patch("app.worker.run_agent", _fake_run_agent):
        returned = await ex.run_delivery_child(
            "goal-1", "scout", {"node_id": "scout", "produces": {"class": "research"}},
            cancel_event=asyncio.Event(), goal_context="# Goal: G",
        )

    assert returned["delivery"] is not None
    assert returned["delivery"]["status"] == "done"
    assert returned["delivery"]["produces"] == "research"
